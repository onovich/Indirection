...
| `@indirection/loaders-web` | `packages/loaders-web/src/index.ts` | Web-friendly JSON/text/binary loaders, ImageBitmap/image-source loader boundary, `BrowserCacheStorageAdapter`, and `MemoryCacheStorageAdapter`. | Runtime core ownership policy, browser APIs inside core packages, Three texture creation, WebGL renderer behavior, or npm publish policy. | `corepack pnpm test -- --run packages/loaders-web`, `corepack pnpm test:browser`, `corepack pnpm test:e2e`, and `corepack pnpm pack:check` |
| `@indirection/three` | `packages/three/src/index.ts` | Optional-peer `model/gltf` loader boundary, parser injection, basePath handling, explicit owned-resource disposal, host-injected texture resource helper, instantiate hook, and animation metadata summary. | Three dependency in core packages, Draco/KTX2/meshopt decoder setup, production renderer framework, automatic deep GPU disposal, scene attach, gameplay factories, or live Sinan integration. | `corepack pnpm test -- --run packages/three`, `corepack pnpm test:e2e`, and `corepack pnpm pack:check` |
...
- Web, Three, and Vite packages are adapters around core contracts, not new sources of truth. Browser ImageBitmap handling belongs in loaders-web, and renderer texture proof stays in Three/browser test boundaries while reaching runtime only through `LoadedAsset.dispose`.
